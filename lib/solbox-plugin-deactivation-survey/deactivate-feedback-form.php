<?php
namespace solbox_wsatc_feedback;
...
if ( defined( 'SB_SCW_DEACTIVATE_FEEDBACK_FORM_INCLUDED' ) ) {
	return;
}
define( 'SB_SCW_DEACTIVATE_FEEDBACK_FORM_INCLUDED', true );
...
add_action(
	...
	function () {
		...
		wp_enqueue_script( 'solbox-deactivate-feedback-wsatc', plugin_dir_url( __FILE__ ) . 'deactivate-feedback-form.js' );
		wp_enqueue_style( 'solbox-deactivate-feedback-wsatc', plugin_dir_url( __FILE__ ) . 'deactivate-feedback-form.css' );

		// Localized strings
		wp_localize_script(
			'solbox-deactivate-feedback-wsatc',
		...
		$plugin->security = wp_create_nonce( 'wsatc-deactivate-nonce' );
		// Send plugin data
		wp_localize_script( 'solbox-deactivate-feedback-wsatc', 'solbox_deactivate_feedback_form_wsatc', $plugins );
	}
);
